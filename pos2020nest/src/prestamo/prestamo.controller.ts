...
//si
    ...
    getAll(@Res() response, @Query('inactivos') incluirInactivos) {
        this.prestamoService.getAll(incluirInactivos).then(( prestamos: Prestamo[] ) => {
            if ( prestamos.length > 0 ) {
                response.status(HttpStatus.OK).json(new CResponse(Status.OK, 'Exito', null, prestamos));
            } else {
                response.status(HttpStatus.OK).json(new CResponse(Status.NO_RECORDS_FOUND, 'No hay zonas registradas pibe', null, null));
            }
        }).catch((error) => {
            response.status(HttpStatus.INTERNAL_SERVER_ERROR)
            .json(new CResponse(Status.ERROR, 'Error al obtener el listado de zonas', null, {}, error));
        });
    }
//si
    @Get(':id')
    get(@Res() response, @Param('id') id: number ) {
        this.prestamoService.getById(id).then(( prestamo: Prestamo ) => {
            if ( prestamo ) {
                response.status(HttpStatus.OK)
                .json(new CResponse(Status.OK, 'Exito', null, prestamo));
            } else {
                response.status(HttpStatus.OK)
                .json(new CResponse(Status.NO_RECORDS_FOUND, `La zona con ID: ${  id.toString() } no existe`));
            }
        }).catch((error) => {
            response.status(HttpStatus.INTERNAL_SERVER_ERROR).json(new CResponse(Status.ERROR, 'Error al obtener la zona', null, {}, error));
        });
    }
//si
    @Post()
    create(@Body() prestamoDto: PrestamoDto, @Res() response) {
        this.prestamoService.create(prestamoDto).then((prestamo) => {
            ...
            response.status(HttpStatus.OK).json(new CResponse(Status.OK, 'Zona creada correctamente', null, prestamo));
            
        }).catch((error) => {
            response.status(HttpStatus.INTERNAL_SERVER_ERROR)
            .json(new CResponse(Status.ERROR, 'Error al crear la nueva zona', null, {}, error));
        });
    }

    @Put(':id')
    update(@Body() prestamoDto: PrestamoDto, @Res() response, @Param('id') id: number) {
